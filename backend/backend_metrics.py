"""Prometheus instrumentation for the Backend service."""
...
TODOS_CREATED = Counter(
    "taskflow_todos_created_total",
    "Todo items stored in the database.",
    ["service"],
    registry=METRICS_REGISTRY,
)
...
def record_worker_failure():
    """Count one failed Worker notification."""
    DEPENDENCY_FAILURES.labels(service=SERVICE, dependency=DEPENDENCY_WORKER).inc()


def record_s3_failure():
    """Count one failed Amazon S3 upload."""
    DEPENDENCY_FAILURES.labels(service=SERVICE, dependency=DEPENDENCY_S3).inc()


def record_postgres_failure():
    """Count one failed PostgreSQL connection."""
    DEPENDENCY_FAILURES.labels(service=SERVICE, dependency=DEPENDENCY_POSTGRES).inc()


def record_todo_created():
    """Count one todo item written to the database."""
    TODOS_CREATED.labels(service=SERVICE).inc()
